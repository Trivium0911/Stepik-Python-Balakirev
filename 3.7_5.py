...
В каждом объекте класса MailItem должны формироваться соответствующие локальные атрибуты (с именами: mail_from,
title, content). И дополнительно атрибут is_read (прочитано ли) с начальным значением False.
...
class MailBox:
    ...
    def receive(self, lst_in):
        #lst_in = list(map(str.strip, sys.stdin.readlines()))
        for i in lst_in:
            res = i.split('; ')
            self.inbox_list.append(MailItem(res[0], res[1], res[2]))
...
mail.inbox_list[2].set_read(True)
inbox_list_filtered = list(filter(bool, mail.inbox_list))
print(inbox_list_filtered)
print(bool(mail.inbox_list[0]))
