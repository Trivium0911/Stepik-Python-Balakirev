"""
                                3.4 Методы __add__, __sub__, __mul__, __truediv__ 10
10 из 11 шагов пройдено
23 из 27 баллов  получено
...
class MaxPooling:
    ...
    def __call__(self, mtx):
        rows = len(mtx)
        cols = len(mtx[0]) if rows > 0 else 0
        if rows == 0:
            return [[]]
        if not all(map(lambda x: len(x) == cols, mtx)) or \
            not all(map(lambda row: all(map(lambda x: type(x) in (int, float), row)), mtx)):
            raise ValueError("Неверный формат для первого параметра matrix.")
        ...
        colls_range = (cols - w) // sw + 1
        res = [[0] * colls_range for _ in range(rows_range)]
        for i in range(rows_range):
            for j in range(colls_range):
                s = (x for r in mtx[i * sh: i * sh + h] for x in r[j * sw: j * sw + w])
                res[i][j] = max(s)
