"""
                            5.3 Распространение исключений (propagation exceptions)  5
...
class Test:
    def __init__(self, descr):
        if not 10 <= len(descr) <= 10000:
            raise ValueError('формулировка теста должна быть от '
                             '10 до 10 000 символов')
        self.descr = descr
...
class TestAnsDigit(Test):
    def __init__(self, descr, ans_digit, max_error_digit=0.01):
        if type(ans_digit) not in (int, float) or type(max_error_digit) \
                not in (int, float) or max_error_digit < 0:
            raise ValueError('недопустимые значения аргументов теста')
    ...
    def run(self):
        ans = float(input())
        return (self.ans_digit - self.max_error_digit) <= ans <= \
