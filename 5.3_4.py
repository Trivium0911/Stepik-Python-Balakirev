"""
                        5.3 Распространение исключений (propagation exceptions)  4
...
[min_length; max_length]); chars - строка из набора символов
(хотя бы один из них должен присутствовать в проверяемой строке).
...
[min_length; max_length] и в string присутствует хотя бы один символ из chars. Если хотя бы один из этих критериев не
выполняется, то генерируется исключение командой:
...
class ValidatorString:
    ...
    def is_valid(self, string):
        if type(string) is not str or not (self.min_length <= len(string)
                                           <= self.max_length) or \
        self.chars and not any([i in string for i in self.chars]):
            raise ValueError('недопустимая строка')
...
class LoginForm:
    ...
    def form(self, request):
        if type(request) is not dict or 'login' not in request.keys() or \
                'password' not in request.keys() or \
                not all([type(i) is str for i in request.values()]):
            raise TypeError('в запросе отсутствует логин или пароль')
