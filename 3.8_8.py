"""
                                3.8 Методы __getitem__, __setitem__ и __delitem__   8
...
class TicTacToe:
    def __init__(self):
        self._n = 3
        self.pole = tuple(tuple(Cell() for i in range(self._n)) for j in range(self._n))
    ...
    def __check(self, val):
        if type(val) is not tuple or len(val) != 2:
            raise IndexError('неверный индекс клетки')
        if any(not (0 <= x < self._n) for x in val if type(x) is not slice):
            raise IndexError('неверный индекс клетки')
