"""
                            3.8 Методы __getitem__, __setitem__ и __delitem__  10
...
class SparseTable:
    ...
    def upd_index(self):
        self.rows = max(key[0] for key in self.dct) + 1
        self.cols = max(key[1] for key in self.dct) + 1

    def add_data(self, row, col, data):
        self.dct[(row, col)] = data
        self.upd_index()

    def remove_data(self, row, col):
        try:
            del self.dct[(row, col)]
            self.upd_index()
        except KeyError:
            raise IndexError('ячейка с указанными индексами не существует')
    ...
    def __getitem__(self, item):
        try:
            return self.dct[(item[0], item[1])].value
        except KeyError:
            raise ValueError('данные по указанным индексам отсутствуют')
