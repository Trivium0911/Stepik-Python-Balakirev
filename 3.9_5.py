"""
                                            3.9 Методы __iter__ и __next__  5
...
class Person:
    def __init__(self, fio, job, old, salary, year_job):
        ...
        self.__len_attrs = len(self._attrs)
        self._iter_index = -1

    def _check_index(self, index):
        if type(index) is not int and not (-self.__len_attrs <= index < self.__len_attrs):
            raise IndexError('неверный индекс')
    ...
    def __next__(self):
        if self._iter_index < self.__len_attrs - 1:
            self._iter_index += 1
