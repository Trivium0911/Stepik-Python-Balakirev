"""
                                            4.8 Испытание "Бремя наследия" 1
...
class LinkedGraph:
    ...
    def add_link(self, link):
        t = tuple(filter(lambda x: (id(x.v1) == id(link.v1) and
                                    id(x.v2) == id(link.v2)) or
                                   (id(x.v2) == id(link.v1) and
    ...
    def _next(self, current, link_prev, current_path, current_links):
        ...
        for link in current.links:
            ...
            if link.v1 not in current_path:
                path, links = self._next(link.v1, link, current_path[:],
                                         current_links[:])
            elif link.v2 not in current_path:
                path, links = self._next(link.v2, link, current_path[:],
                                         current_links[:])

            if self._stop_v in path and (len_path > self._dist_path(links)
                                         or len_path == -1):
